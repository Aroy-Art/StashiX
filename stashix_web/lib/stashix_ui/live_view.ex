defmodule StashixUi.LiveView do
  @moduledoc """
  ...
  def handle_event("open_profile", _params, socket) do
    socket = StashixUi.LiveView.send_command(socket, "profile-sheet", "open")
    {:noreply, socket}
  end

  def handle_event("close_profile", _params, socket) do
    socket = StashixUi.LiveView.send_command(socket, "profile-sheet", "close")
    {:noreply, socket}
  end
  ```
  ...
  Use `StashixUi.JS.dispatch_command/3` when a LiveView JS command should control a
  component directly in the browser.

  ```heex
  <.button phx-click={%JS{} |> StashixUi.JS.dispatch_command("open", to: "#profile-sheet")}>
    Open
  </.button>
  ```
  ...
  ```elixir
  socket = StashixUi.LiveView.send_command(socket, "dialog", "open")
  ```

  With params:

  ```elixir
  socket = StashixUi.LiveView.send_command(socket, "chart", "update", %{series: data})
  ```
  """
...
defmodule StashixUi.JS do
  ...
  commands sent by `StashixUi.LiveView.send_command/4`.

  ## Example

  ```heex
  <.button phx-click={%JS{} |> StashixUi.JS.dispatch_command("open", to: "#dialog")}>
    Open dialog
  </.button>
  ```
  ...
  <.button
    phx-click={
      %JS{}
      |> StashixUi.JS.dispatch_command("update", to: "#chart", detail: %{series: @series})
    }
  >
    Update chart
  </.button>

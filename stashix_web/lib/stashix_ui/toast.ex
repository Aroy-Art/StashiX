defmodule StashixUi.Toast do
  ...
  Use the helpers below from any `handle_event/3`:
      ...
      def handle_event("save", _params, socket) do
        {:noreply, StashixUi.Toast.toast_success(socket, "Saved!")}
      end
  ...
  Declare a `:template` next to the toaster and reference it by name:
      ...
      StashixUi.Toast.toast(socket, template: "new-follower")
  ...
  Per-toast, overriding the toaster-wide default for just this call:
      ...
      StashixUi.Toast.toast_success(socket, "Saved!",
  ...
  `!`-prefixed utilities there to reliably win the cascade:
      ...
      StashixUi.Toast.toast(socket, "Something broke",
  ...
  * `:on-dismiss` - Fired whenever a toast is removed, with `%{id:, reason:}` in the payload.
  ...
  list/map of opts with no title:
      ...
      socket = StashixUi.Toast.toast(socket, "Saved!")
      socket = StashixUi.Toast.toast(socket, "Saved!", description: "Changes are live")
      socket = StashixUi.Toast.toast(socket, template: "new-follower", variant: "default")
  ...
  def toast(socket, toaster_id, message, opts) do
    StashixUi.LiveView.send_command(socket, toaster_id, "add", build_toast_params(message, opts))
  end
  ...
  def toast_update(socket, id, opts) do
    ...
    StashixUi.LiveView.send_command(socket, "toaster", "update", params)
  end
  ...
  def toast_dismiss(socket, toaster_id \\ "toaster", id \\ nil) do
    ...
    StashixUi.LiveView.send_command(socket, toaster_id, "dismiss", params)
  end

...
import { setAuthTokens } from '@/lib/auth-cookie'
...
function LibraryStep({ token: _token }: { token: string }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true)
    try {
      await librariesApi.create({ name, root_path: path, standalone_folders: sfFolders })
      await restore()
      router.push('/')
    } catch (err) {
